...
const menuHierarchy: MenuHierarchy[] = [
  ...
  { key: 'management', subItems: [
    'manage-users', 'manage-technicians', 'manage-franchises', 'manage-executives', 'manage-referrals'
  ]},
...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
  ...
  const getNestedSubItems = (parentItem: MenuItem): MenuItem[] => {
    return menuItems.filter(item => item.path.startsWith(parentItem.path + '/') && item.path !== parentItem.path);
  };
              ...
              {hasSubItems && openMenus[parent.key] && (
                <div className="space-y-1">
                  {parent.subItems.map((subKey) => {
                    ...
                    const nestedSubItems = isNestedParent ? getNestedSubItems(subItem) : [];
