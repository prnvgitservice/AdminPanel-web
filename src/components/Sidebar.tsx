...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, onPageChange, onClose }) => {
  ...
  return (
    <div className="w-64 bg-white/95 backdrop-blur-md shadow-2xl h-full flex flex-col border-r border-slate-200/50">
      ...
      <nav className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                  ...
                  <div key={item.id}>
                    ...
                    >
                      ...
                      {item.hasSubmenu && !item.isSubItem && (
                        <ChevronRight className={`h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0 transition-all duration-200 ${
                          currentPage === item.id 
                            ? 'text-white' 
