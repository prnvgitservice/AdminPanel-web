...
const AllAreas = () => {
  ...
  useEffect(() => {
    setTotal(filteredPincodes.length);
    setOffset(0); // Reset offset when search changes
  }, [filteredPincodes.length]);

  const handleEdit = (id: string) => {
    const pincode = allPincodes.find((p) => p._id === id);
  ...
  const handleDelete = async (pincode: Pincode) => {
    if (window.confirm("Are you sure you want to delete this pincode?")) {
      try {
        const response = await deletePincode(pincode.code);
  ...
  const displayedPincodes = filteredPincodes.slice(offset, offset + limit);
