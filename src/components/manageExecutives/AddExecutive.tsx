...
import { getAllPincodes, registerExecutiveByAdmin,  } from '../../api/apiMethods';
...
const initialFormState: ExecutiveData = {
  executivename: '',
  phoneNumber: '',
  password: '',
...
const AddExecutive: React.FC = () => {
  const [formData, setFormData] = useState<ExecutiveData>(initialFormState);
  ...
  useEffect(() => {
    if (selectedPincode) {
      ...
      if (found && found.areas) {
        const areas = found.areas.map(area => ({ _id: area._id, name: area.name }));
  ...
  const validateForm = useCallback((): FormErrors => {
    ...
    if (!formData.executivename.trim()) {
      newErrors.executivename = 'Name is required.';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await registerExecutiveByAdmin(formData);
      if (response.success) {
        ...
        setTimeout(() => {
          navigate('/management/executives/all');
        }, 2000);
      } else {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Add Executive</h1>
          </div>
          <button
            onClick={() => navigate(-1)}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-500 to-blue-600 px-6 py-4">
              <h2 className="text-lg font-semibold text-white">Executive Information</h2>
            </div>

            <div className="p-6 space-y-6">
              {submissionStatus === 'success' && (
                <div className="text-green-600 text-sm text-center bg-green-50 p-2 rounded">
                  Executive added successfully! Redirecting...
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    required
                    aria-describedby={errors.executivename ? 'executivename-error' : undefined}
                    disabled={isSubmitting}
                  />
                  {errors.executivename && (
                    <p id="executivename-error" className="text-red-500 text-sm">
                      {errors.executivename}
                    </p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="trainee marketing executive">Trainee Marketing Executive</option>
                    <option value="jr. marketing executive">Jr. Marketing Executive</option>
                    <option value="senior marketing executive">Senior Marketing Executive</option>
                  </select>
                  {errors.role && (
                    <p id="role-error" className="text-red-500 text-sm">
                      {errors.role}
                    </p>
                  )}
                </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Processing...' : 'Add Executive'}
            </button>
          </div>
        </form>
      </div>
...
// import { ArrowLeft, Briefcase } from 'lucide-react';
// import { FaEye, FaEyeSlash } from 'react-icons/fa';
// import { registerExecutiveByAdmin, getAllPincodes } from '../../api/apiMethods';
...
//       const found = pincodeData.find((p) => p.code === selectedPincode);
//       if (found && found.areas) {
//         const areas = found.areas.flatMap(area => [
//           { _id: area._id, name: area.name },
//           ...area.subAreas.map(subArea => ({ _id: subArea._id, name: subArea.name }))
//         ]);
//         setAreaOptions(areas);
//         setFormData((prev) => ({
//           ...prev,
...
// const registerExecutive = async (data: ExecutiveData) => {
...
//     try {
//       await registerExecutive(formData);
//       setSubmissionStatus('success');
