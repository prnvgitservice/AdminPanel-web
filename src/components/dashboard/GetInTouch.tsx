...
import { Booking, RecentGetInTouchResponse } from '../../pages/Dashboard';
import { getRecentGetInTouchDashboard } from '../../api/apiMethods';
...
const GetInTouch: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (getInTouchResponse.success) {
          ...
          setError('Failed to fetch get in touch data');
        }
      } catch (error) {
        setError('Error fetching get in touch data');
  ...
  if (loading) {
    return <div className="text-center p-4">Loading get in touch...</div>;
  }
