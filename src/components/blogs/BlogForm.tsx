...
const BlogForm: React.FC<BlogFormProps> = ({ isEdit = false }) => {
  ...
  const editorRef = useRef<any>(null); // JoditEditor ref (type 'any' for simplicity; use @types/jodit-react for better typing)
  ...
  // JoditEditor configuration (memoized)
  const config = {
  readonly: false,
  ...
  // useEffect(() => {
  //   const errors = {
  //     title: formData.title.trim() ? '' : 'Blog title is required',
  ...
  const handleTagKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && tagInput.trim()) {
      ...
      if (trimmedTag.length < 1 || trimmedTag.length > 50) {
        setFieldErrors((prev) => ({ ...prev, tags: 'Tags must be between 1 and 50 characters' }));
        return;
      }
      ...
      if (editingTagIndex !== null) {
        setFormData((prev) => ({
          ...prev,
          tags: prev.tags.map((tag, i) => (i === editingTagIndex ? trimmedTag : tag)),
        }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = isEdit && initialBlog?._id
        ? await updateBlog(formDataPayload)
        : await createBlog(formDataPayload);

      if (response.success) {
        navigate('/blogs/all');
      } else {
        setError(response.message || (isEdit ? 'Failed to update blog post' : 'Failed to create blog post'));
      }
    } catch (err: any) {
      setError(err.message || (isEdit ? 'Error updating blog post. Please try again.' : 'Error creating blog post. Please try again.'));
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-6">
            ...
            <div>
              ...
              />
              {fieldErrors.title && (
                <p id="title-error" className="text-red-500 text-sm mt-1 animate-slide-in">
                  {fieldErrors.title}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {formData.tags.map((tag, index) => (
                  <span
                    ...
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => e.key === 'Enter' && editTag(index)}
                  >
                    {tag}
                    <button
                      type="button"
              ...
              />
              {fieldErrors.tags && (
                <p id="tags-error" className="text-red-500 text-sm mt-1 animate-slide-in">
                  {fieldErrors.tags}
                </p>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div
                className={`border rounded-lg ${fieldErrors.description ? 'border-red-500' : 'border-gray-300'}`}
              >
                <JoditEditor
                  ref={editorRef}
              ...
              {fieldErrors.description && (
                <p id="description-error" className="text-red-500 text-sm mt-1 animate-slide-in">
                  {fieldErrors.description}
                </p>
              )}
            </div>
          ...
          <div className="flex items-center justify-between space-x-4 pt-8 border-gray-200">
            ...
            <button
              ...
              disabled={isSubmitting}
              aria-label={isEdit ? 'Update blog' : 'Create blog'}
            >
              {isSubmitting ? (
