...
import { Mail, Search, Eye, X, ChevronLeft, ChevronRight } from 'lucide-react';
import { getInTouch, updateGetInTouchStatus } from '../../api/apiMethods';
...
const GetInTouch: React.FC = () => {
  ...
  const handleStatusChange = async (contactId: string, currentStatus: string) => {
    try{
      if(window.confirm("Are you sure you want to mark this contact as completed?")){
        if (currentStatus.toLowerCase() === 'pending') {
          const response = await updateGetInTouchStatus({ bookingId: contactId, status: 'completed' });
          if (!response?.success) {
            throw new Error(response?.message || "Failed to update contact status");
          }
          if(response?.success){ 
            alert(response?.message || "Contact status updated to completed.");
            // Refresh contacts to reflect the updated status
            fetchContacts();
          }
        }
      }
  ...
  const fetchContacts = useCallback(
    async () => {
      try {
        ...
        const response: ApiResponse = await getInTouch(params);

        if (!response.success) {
          throw new Error(response.message || "Failed to fetch contacts");
        }

        setContacts(response.result.touchs || []);
        setTotalContacts(response.result.totalTouch || 0);
      } catch (err: unknown) {
        const errorMessage =
    ...
    [offset, limit, searchTerm, statusFilter]
  );
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {displayedContacts.map((contact) => (
                    <tr key={contact._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {contact.categoryName || contact.categoryId || '-'}
                        </div>
                      </td>
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-base text-gray-900 font-medium">{selectedContact.categoryName || selectedContact.categoryId || '-'}</p>
                  </div>
...
//   const displayedContacts = filteredContacts.slice(offset, offset + limit);
...
//         if (currentStatus.toLowerCase() === 'pending') {
//           const response = await updateGetInTouchStatus({ bookingId, status: 'completed' });
//           if (!response?.success) {
