...
import { getAllFranchiseRequests } from "../../api/apiMethods";
...
const FranchiseRequest = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        setLoading(true);
        const response = await getAllFranchiseRequests();
        setRequests(response?.result);
        setTotal(response?.result?.length);
      } catch (err: any) {
        setError(err.message || "Failed to fetch franchise requests");
      } finally {
        setLoading(false);
      }
    };
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50 text-left">
                  <tr>
                    ...
                    <th className="px-6 py-3 w-2/6">Message</th>
                    <th className="px-6 py-3 w-1/6">Requested Date</th>
                  ...
                  {requests
                    .filter(
                      (request) =>
                        ...
                        request.phoneNumber.toString().includes(searchTerm)
                    )
